    ...
    for x in range(0, board.height):
        for y in range(0, board.width):
            ...
            if square_object is None:
                rect = pygame.draw.rect(display, BLACK, [x*square_width ,y * square_height, square_width, square_height])
            elif isinstance(square_object, Cube):
                rect =  pygame.draw.rect(display, WHITE, [x*square_width ,y * square_height, square_width, square_height])
